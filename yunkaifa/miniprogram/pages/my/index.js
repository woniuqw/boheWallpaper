...
Page({
  ...
  topictureDeail:function(e){
    var categeryname = e.currentTarget.dataset.categerylist;
    wx.navigateTo({
      url: '../categorypictures/categorypictures?categeryname=' + categeryname,
  ...
  async categorylist(){
    ...
    // 查询当前用户所有的 counters
    db.collection('categorylist').where({
      //  _openid: app.globalData.openid
    }).skip(pageIndex).limit(20).get({
      success: res => {
        if (res.data.length !=0) {
          var arr1 = that.data.categorylist; //从data获取当前datalist数组
